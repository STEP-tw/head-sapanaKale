...
describe("validateInput", function () {
    it("should return nothing when provided option and count is valid for given context", function () {
        assert.deepEqual(validateInput({ option : 'line', count : 1 }, "head"), '');
        assert.deepEqual(validateInput({ option : 'line', count : 1 }, "tail"), '');
        assert.deepEqual(validateInput({ option : 'byte', count : 2 }, "head"), '');
        assert.deepEqual(validateInput({ option : 'byte', count : 2 }, "tail"), '');
    });
    
    it("should return nothing when given count is 0 and context is tail", function () {
        assert.deepEqual(validateInput({ option : 'line', count : 0 }, "tail"), '');   
        assert.deepEqual(validateInput({ option : 'byte', count : 0 }, "tail"), '');   
    });

    it("should return invalid count error when given count is 0 and context is head", function () {
        let actualOutput = validateInput({ option : 'line', count : 0 }, "head");
        let expectedOutput = "head: illegal line count -- 0";
        assert.deepEqual(actualOutput, expectedOutput);
    });
    
    it("should return invalid count error when count is not +ve number and context is head", function () {
        let actualOutput = validateInput({ option : 'line', count : -1 }, "head");
        let expectedOutput = "head: illegal line count -- -1";
        assert.deepEqual(actualOutput, expectedOutput);

        actualOutput = validateInput({ option : 'byte', count : "c2" }, "head");
    ...
    it("should return invalid count error when count is not +ve number and context is tail", function () {
        let actualOutput = validateInput({ option : 'line', count : -1 }, "tail");
        let expectedOutput = "tail: illegal offset -- -1";
        assert.deepEqual(actualOutput, expectedOutput);

        actualOutput = validateInput({ option : 'byte', count : "c2" }, "tail");
